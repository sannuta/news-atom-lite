...
Usage:
  python extract.py --url https://... --backend openai --org "My Publication"
  python extract.py --file article.txt --backend anthropic --org "My Publication"
  python extract.py --file article.txt --backend ollama --model llama3.2 --org "My Publication"
...
Output:
  By default, output is appended to events.jsonl and atoms.jsonl.
  Use --overwrite to replace existing output files instead.
"""
...
examples:
  python extract.py --url https://example.com/article --backend openai --org "My Publication"
  python extract.py --file article.txt --backend anthropic --org "My Publication"
  python extract.py --file article.txt --backend ollama --model llama3.2 --org "My Publication"
  python extract.py --file article.txt --backend huggingface --model sannuta/newsatom-gemma-2b --gemma-format
        """
    ...
    parser.add_argument("--org", default="",
                        help="Publisher name — used to derive ID prefix (e.g. 'Scroll.in' → SCR)")
    ...
    parser.add_argument(
        ...
        help="Overwrite existing output files. Default is to append (safe for pipelines)."
    )
    ...
    # Derive org_prefix once — used by both counter and wrapper
    org_prefix = args.org_prefix if args.org_prefix else make_prefix(org)

    if not org and not args.org_prefix:
        print(
            "⚠  Warning: no --org provided. IDs will use prefix 'UNK'. "
            "Pass --org 'Publisher Name' for meaningful IDs.\n"
        )

    prompt = build_prompt(
        sentences=sentences,
        pub_date=date, url=url, title=title,
        journalist=journalist, org=org,
    ...
    with open(events_path, mode) as f:
        for event in events:
            f.write(json.dumps(event, ensure_ascii=False) + "\n")

    with open(atoms_path, mode) as f:
        for atom in atoms:
