...
def load_counters(output_dir: str) -> dict:
    """Load counter state from the output directory."""
...
def save_counters(output_dir: str, counters: dict) -> None:
    """Save counter state to the output directory."""
    path = Path(output_dir) / COUNTER_FILENAME
...
def get_next_counters(
    ...
    """
    Reserve a block of IDs for this extraction run.

    Returns (event_start, atom_start) — the first ID to use for
    this batch of events and atoms.

    Example:
        If the counter file shows SCR-EVT last used = 42,
        and this run has 5 events, returns event_start=43.
        Counter file is then updated to last_event=47.
    """
    ...
    # Reserve the block
    prefix_counters["last_event"] = prefix_counters["last_event"] + n_events
    prefix_counters["last_atom"] = prefix_counters["last_atom"] + n_atoms
...
def reset_counters(output_dir: str, org_prefix: str = None) -> None:
    """
    Reset counters. If org_prefix given, reset only that prefix.
